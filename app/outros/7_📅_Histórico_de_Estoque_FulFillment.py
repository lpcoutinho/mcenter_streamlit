...
st.set_page_config(
    page_title="Histórico de Estoque Fulfillment", page_icon="", layout="wide"
)
...
df_filtrado = df[
    (df["created_at"] >= pd.to_datetime(data_inicio))
    & (df["created_at"] <= pd.to_datetime(data_fim))
]
...
df_top_10 = (
    df.sort_values(by="available_quantity", ascending=False)
    .drop_duplicates("ml_inventory_id")
    .head(10)
)
