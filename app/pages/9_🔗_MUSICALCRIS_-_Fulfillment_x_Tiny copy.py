...
logger.add(
    "Data/Output/Log/cris_ful_x_tiny.log",
    rotation="10 MB",
...
st.set_page_config(
    page_title="MUSICALCRIS Fullfillment x Tiny", page_icon="📦", layout="wide"
)
...
if "resultado_busca_cris" not in st.session_state:
    st.session_state.resultado_busca_cris = pd.DataFrame()
...
if st.sidebar.button("Buscar Dados"):
    if valor_busca:
        ...
        st.session_state.resultado_busca_cris = df[df[parametro_busca] == valor_busca]
        st.header("Resultados da Busca")
        st.dataframe(st.session_state.resultado_busca_cris)
    else:
...
if option == "Atualizar Dados":
    ...
    for index, row in st.session_state.resultado_busca_cris.iterrows():
        st.write(
    ...
    st.header("MUSICALCRIS: Relação Fulfillment x Tiny")
