...
class MeLiLoader:
    ...
    def load_fulfillment_stock_hist(self):
        logger.info(
            f"Carregando histórico de produtos disponíveis no fulfillment"
        )
        try:
            conn = psycopg2.connect(
                **self.db_config
    ...
    def get_orders_data(self):
        ...
        df_orders['item.variation_id'] = df_orders['item.variation_id'].astype(str).apply(lambda x: x.split('.')[0] if '.' in x else x)
        ...
        cols = {'item.id':'ml_code', 'item.category_id':'category_id', 'item.variation_id':'variation_id','item.seller_sku':'seller_sku', 'item.title':'title'}
    ...
    # def calculate_fulfillment_metrics(self, df_unido, date_from, date_to):
        ...
        df_unido['today_send_fulfillment'] = df_unido['today_send_fulfillment'].fillna(0)
        ...
        df_unido['period_send_fulfillment'] = df_unido['period_send_fulfillment'].fillna(0)
    ...
    def insert_orders_data(self, df_orders):
        ...
        try:
            ...
            for index, row in df_orders.iterrows():
                ...
                cursor.execute(
                    insert_query, (row["ml_code"], row["category_id"], row["variation_id"], row["seller_sku"],row["pack_id"], row["quantity"], row["title"], row["order_status"], row["payment_status"], row["shipping_id"], row["date_approved"], row["date_closed"])
                )
