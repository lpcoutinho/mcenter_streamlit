...
st.set_page_config(page_title="MCENTER ITENS", page_icon="📦", layout="wide")
...
# Consulta SQL
query = "SELECT * FROM mcenter_items;"

# Recupera os dados usando a função fetch_data
df = fetch_data(query)

st.header("MCENTER Itens")
...
with col2:
    if st.button("Atualizar banco de dados", type="primary"):
        ...
        # Consulta tabela mcenter_items do db
        try:
            conn = psycopg2.connect(**db_config)
            ...
            logger.error(f"Erro do psycopg2 em 'mcenter_items': {e}")
        except Exception as e:
            logger.error(f"Erro ao consultar 'mcenter_items': {e}")
        ...
        for index, row in df_atualizado_sem_nan.iterrows():
            ...
            update_query = sql.SQL(
                "UPDATE mcenter_items SET value_name = %s, status = %s, catalog_listing = %s, updated_at = %s WHERE ml_code = %s AND inventory_id = %s"
            )
        ...
        cursor.close()
        conn.close()
        logger.info("Dados atualizados com sucesso em mcenter_items!")
        ...
        for index, row in diferenca.iterrows():
            insert_query = sql.SQL(
                "INSERT INTO mcenter_items (ml_code, inventory_id, value_name, status, catalog_listing) VALUES (%s, %s, %s, %s, %s)"
            )
            cursor.execute(
                insert_query,
                (
        ...
        cursor.close()
        conn.close()
        print("Novos dados inseridos com sucesso em mcenter_items!")
