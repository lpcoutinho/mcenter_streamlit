...
df_filtrado = df[(df['date_closed'] >= pd.to_datetime(data_inicio)) & (df['date_closed'] <= pd.to_datetime(data_fim))]

# Exibir DataFrame filtrado
st.write('DataFrame Filtrado:', df_filtrado)

# Reduzir a quantidade de IDs ou pegar os 10 maiores disponíveis
df_top_10 = df.sort_values(by='quantity', ascending=False).drop_duplicates('ml_code').head(10)
